...
import { Checked, CircleClose } from '@tu-view-plus/icons-vue'
...
describe('Switch', () => {
  ...
  test('create', () => {
    const wrapper = mount(() => (
      <Switch activeText="on" inactiveText="off" width={100} />
    ));
    const vm = wrapper.vm;

    expect(vm.$el.classList.contains('is-checked')).false;
    const coreEl = vm.$el.querySelector('.tu-switch__core');
    expect(coreEl.style.width).toEqual('100px');
    const leftLabelWrapper = wrapper.find('.tu-switch__label--left span');
    expect(leftLabelWrapper.text()).toEqual('off');
  });

  test('size', () => {
    const wrapper = mount(() => <Switch size="large" />);
    expect(wrapper.find('.tu-switch--large').exists()).toBe(true);
  });

  test('tabindex', () => {
    const wrapper = mount(() => <Switch tabindex="0" />);
    expect(wrapper.find('.tu-switch__input').attributes().tabindex).toBe('0');
  });
  ...
  test('inline prompt', () => {
    ...
    const coreEl = vm.$el.querySelector('.tu-switch__core')
    expect(coreEl.style.width).toEqual('100px')
    const label = wrapper.find('.tu-switch__inner span')
  ...
  test('value correctly update', async () => {
    ...
    const coreWrapper = wrapper.find('.tu-switch__core')
  ...
  test('change event', async () => {
    ...
    const value = ref(true)
    const handleChange = (val: string | number | boolean) => target.value = val
    const wrapper = mount(() => (
