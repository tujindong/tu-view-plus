...
import { getStyle } from '@tu-view-plus/utils'
import { rAF } from '@tu-view-plus/test-utils/tick'
import { TuMessage } from '../index'
import Message from '../src/methods'

const selector = '.tu-message'
...
describe('Message on command', () => {
  ...
  describe('context inheritance', () => {
    it('should globally inherit context correctly', () => {
      expect(TuMessage._context).toBe(null)
      const testContext = {
        config: {
          globalProperties: {},
        },
      ...
      TuMessage.install?.(testContext as any)
      expect(TuMessage._context).not.toBe(null)
      expect(TuMessage._context).toBe(testContext._context)
      TuMessage._context = null
    })
  })
})
